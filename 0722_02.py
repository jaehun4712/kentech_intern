...
soc_ocv_data = ocv_raw.iloc[start_row:, [6, 7]] #C_rate=0.05C로 수정
...
discharge_mask = df['valid'] & ((df[cable_col] == 0) | (df[speed_col] > 0)) & (df[current_col] > 0)
...
for seg_id, group in disch_groups:
    ...
    if abs(soc_start - soc_end) < 0.0005:
        continue  # SOC 변화 거의 없으면 건너뜀
